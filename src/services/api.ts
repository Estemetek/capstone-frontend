...
// 🟢 NEW: Fetch single beneficiary by ID
export async function getBeneficiary(id: string): Promise<Beneficiary> {
  return authFetch(`${API_BASE_URL}/beneficiaries/${id}`);
}
...
// -----------------------------
// Fetch a specific user's details by ID (used for Admin/DonorDetails)
export async function getDonorDetails(id: string): Promise<User> {
  return authFetch(`${API_BASE_URL}/users/donors/${id}`);
}
